...
if __name__ == "__main__":
    action_bound = None # 本场景没有连续动作，所以为 None

    # =========================================================================
    # 重构点：在 Main 中实例化网络和 Wrapper
    # =========================================================================
    ...
    # 2. 使用 Wrapper 包装 Actor (提供统一接口)
    # 注意：如果 action_dims_dict 中包含 'cont'，必须提供 action_bound
    actor_wrapper = HybridActorWrapper(actor_net, action_dims_dict, action_bound, device).to(device)

    # 3. 将 Wrapper 和 Critic 传入 PPO 算法
    agent = PPOHybrid(actor=actor_wrapper, 
                      critic=critic_net, 
                      actor_lr=actor_lr, 
                      critic_lr=critic_lr,
                      lmbda=lmbda, 
    ...
    # 注意：保存 meta 时，agent.actor 现在是 Wrapper，agent.actor.net 才是原始网络
    save_meta_once(actor_meta_path, agent.actor.net.state_dict())
    save_meta_once(critic_meta_path, agent.critic.state_dict())

    from Math_calculates.ScaleLearningRate import scale_learning_rate

    # 根据参数数量缩放学习率
    actor_lr = scale_learning_rate(actor_lr, agent.actor.net)
    ...
    try:
        ...
        while total_steps < int(max_steps * (1 - pre_train_rate)):
            ...
            for count in range(round(args.max_episode_len / dt_maneuver)):
                ...
                # 发射导弹判决
                u, _ = agent.take_action(b_obs_n, explore=1) # 0 1
                ut = u['bern'][0]
                at = ut
                ...
                done, b_reward, _ = env.get_terminate_and_reward('b', ut)
            ...
            logger.add("train/3 actor_grad_norm", agent.actor_grad, total_steps)
            logger.add("train/5 actor_pre_clip_grad", agent.pre_clip_actor_grad, total_steps)
            logger.add("train/4 critic_grad_norm", agent.critic_grad, total_steps)
            logger.add("train/6 critic_pre_clip_grad", agent.pre_clip_critic_grad, total_steps)
