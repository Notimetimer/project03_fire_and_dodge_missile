...
from Envs.Tasks.ChooseStrategyEnv2_2_hierarchical_new import *
    ...
    try:  # <--- 【新增】添加此行，并将下方所有代码整体缩进
        ...
        while True:
            ...
            if cmd == 'RUN_EPISODE':
                ...
                acc_reward_for_learn = 0
                acc_reward_for_enm = 0
                ...
                for count in range(max_counts):
                    ...
                    if steps_run % action_cycle_multiplier == 0:
                        ...
                        if steps_run > 0:
                            ...
                            append_experience(local_trans, last_decision_obs, last_decision_state, current_action, acc_reward_for_learn, b_state_global, False, not dead_dict['b'])
                            append_experience(ego_trans, last_decision_obs, last_decision_state, current_action_exec, acc_reward_for_learn, b_state_global, False, not dead_dict['b'])
                            append_experience(enm_trans, last_enm_decision_obs, last_enm_decision_state, current_enm_action_exec, acc_reward_for_enm, r_state_global, False, not dead_dict['r'])
                    ...
                    # 4. 奖励计算
                    b_fired_this_step = (b_m_id is not None) if (steps_run - 1) % action_cycle_multiplier == 0 else False
                    r_fired_this_step = (r_m_id is not None) if (steps_run - 1) % action_cycle_multiplier == 0 else False
                    
                    done, b_rew_event, b_rew_constraint, b_rew_shaping = env.combat_terminate_and_reward('b', b_action_label, b_fired_this_step, action_cycle_multiplier)
                    _, r_rew_event, r_rew_constraint, r_rew_shaping = env.combat_terminate_and_reward('r', r_action_label, r_fired_this_step, action_cycle_multiplier)
                    ...
                    acc_reward_for_learn += reward_for_learn
                    acc_reward_for_enm += reward_for_enm
                ...
                if last_decision_state is not None:
                    append_experience(local_trans, last_decision_obs, last_decision_state, current_action, acc_reward_for_learn, next_b_state_global, True, not dead_dict['b'])
                    append_experience(ego_trans, last_decision_obs, last_decision_state, current_action_exec, acc_reward_for_learn, next_b_state_global, True, not dead_dict['b'])
                    append_experience(enm_trans, last_enm_decision_obs, last_enm_decision_state, current_enm_action_exec, acc_reward_for_enm, next_r_state_global, True, not dead_dict['r'])
...
def run_MLP_simulation(
    ...
    while True:
        while total_steps < current_max_steps:
            action_cycle_multiplier = int(max(10, 30 - 10 * (total_steps / 4e6))) # 6s 缩小到2s
