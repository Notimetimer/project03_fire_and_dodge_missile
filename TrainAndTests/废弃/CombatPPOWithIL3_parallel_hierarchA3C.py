...
def restructure_actions(actions_data):
    ...
    并确保维度是 (N, 1) 以适配 A3CHybrid 的索引操作
    """
    # 如果已经是字典格式，直接返回
    ...
    if isinstance(actions_data, list) and len(actions_data) > 0:
        ...
        # 这一点至关重要：A3CHybrid 里的 expert_cat[:, i] 需要 expert_cat 是二维的
...
def worker_process(rank, pipe, args, state_dim, hidden_dim, 
    ...
    try:  # <--- 【新增】添加此行，并将下方所有代码整体缩进
        ...
        # 【修改 1】创建一个 dummy critic，仅为了满足 A3CHybrid 初始化要求
        local_dummy_critic = ValueNet(state_dim, hidden_dim).to(device_worker)
        local_agent = A3CHybrid(
        ...
        while True:
            ...
            if cmd == 'RUN_EPISODE':
                ...
                env.reset(red_birth_state=red_birth, blue_birth_state=blue_birth, red_init_ammo=4, blue_init_ammo=4, pomdp=1)
