...
from Envs.Tasks.ChooseStrategyEnv2_0 import ChooseStrategyEnv
from BasicRules import basic_rules
from Envs.battle6dof1v1_missile0919 import launch_missile_immediately
from Algorithms.PPOHybrid23_1 import PolicyNetHybrid, HybridActorWrapper, ValueNet
...
if __name__ == "__main__":
    ...
    # --- 环境和模型参数 (必须与训练时一致) ---
    env_args = argparse.Namespace(max_episode_len=15*60, R_cage=55.00e3)
            ...
            for count in range(round(env_args.max_episode_len / dt_maneuver)):
                ...
                if count % action_cycle_multiplier == 0:
                    ...
                    with torch.no_grad():
                        ...
                        b_action_exec, b_action_raw, h_actor_next, b_action_check = actor_wrapper.get_action(b_obs, h=h_actor, \
