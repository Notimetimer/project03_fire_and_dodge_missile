...
if __name__=="__main__":
    ...
    v = np.linspace(0.9 * 340, 2 * 340, 5)

    # alt: 3000到12000，10个等间隔
    alt = np.linspace(3000, 12000, 10)
    ...
    # D: 0到2pi（不含2pi），7个等间隔
    D = np.linspace(0, 2 * np.pi, 8)[:-1]  # shape=(7,)
    Delta_theta = np.pi / 2 * np.cos(D)
    Delta_psi = np.pi / 2 * np.sin(D)
    # ATA: 40到60，步长1
