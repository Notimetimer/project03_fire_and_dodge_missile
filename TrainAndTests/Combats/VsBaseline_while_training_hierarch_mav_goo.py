            ...
            if steps % action_cycle == 0:
                ...
                with torch.no_grad():
                    ...
                    if deterministic:
                        ...
                        b_mav_exec, _, _, _ = mav_actor.get_action(b_obs, explore=explore_dict, temperature=Temperature)
                        b_goo_exec, _, _, _ = goo_actor.get_action(b_obs, explore=explore_dict, temperature=Temperature, check_obs=b_check)
                    else:
                        if Temperature is None:
                            Temperature = {'cat':1, 'bern':1}
                        explore_dict = {'cont': 1, 'cat': 1, 'bern': 1}
                        b_mav_exec, _, _, _ = mav_actor.get_action(b_obs, explore=explore_dict, temperature=Temperature)
                        b_goo_exec, _, _, _ = goo_actor.get_action(b_obs, explore=explore_dict, temperature=Temperature, check_obs=None)
