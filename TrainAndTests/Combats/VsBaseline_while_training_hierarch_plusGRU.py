            ...
            if steps % action_cycle == 0:
                ...
                with torch.no_grad():
                    ...
                    if deterministic:
                        ...
                        b_act_exec, _, b_h_out, _ = actor.get_action(b_obs, h=actor_h, explore=explore_dict, temperature=Temperature, check_obs=b_check)
                    else:
                        if Temperature is None:
                            Temperature = {'cat':1, 'bern':1}
                        explore_dict = {'cont': 1, 'cat': 1, 'bern': 1}
                        b_act_exec, _, b_h_out, _ = actor.get_action(b_obs, h=actor_h, explore=explore_dict, temperature=Temperature, check_obs=None)
                    actor_h = b_h_out[:, 0, :].detach().cpu().numpy() if b_h_out is not None else None
