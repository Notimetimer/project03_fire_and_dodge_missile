...
from Algorithms.PPOHybridClockWise import PPOHybrid, PolicyNetHybrid, HybridActorWrapper
...
def test_worker(model_state_dict, rule_num, 
    ...
    actor = HybridActorWrapper(net, action_dims_dict, None, device, circ_angles=[0, np.pi/3, np.pi/2, np.pi, -np.pi/2, -np.pi/3], lin_angles=[np.pi/4, np.pi/8, 0.0, -np.pi/8, -np.pi/2]).to(device)
    ...
    for _ in range(num_runs):
        ...
        while not done and test_env.running:
            ...
            if steps % action_cycle == 0:
                ...
                # 蓝方 (神经网络)
                with torch.no_grad():
                    # 如果 deterministic 为 True，则机动采用确定性决策，开火(bern)仍保持随机(1)
                    if deterministic:
                        explore_dict = {'lin':0, 'circ':0, 'bern': 1}
                        b_act_exec, _, _, _ = actor.get_action(b_obs, explore=explore_dict, check_obs=b_check)
                    else:
                        explore_dict = {'lin':1, 'circ':1, 'bern': 1}
