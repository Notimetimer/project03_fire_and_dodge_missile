...
def restructure_actions(actions_data):
    ...
    if isinstance(actions_data, list) and len(actions_data) > 0:
        ...
        result = {'cat': cat_arr, 'bern': bern_arr}
...
def append_experience(td, obs, state, action, reward, next_state, done, active_mask):
    ...
    td['active_masks'].append(active_mask)
    return td
...
def worker_process(rank, pipe, args, state_dim, hidden_dim, 
    ...
    try:
        ...
        local_agent = PPOHybrid(
            ...
            actor_lr=0, critic_lr=0, lmbda=0, eps=0, gamma=0, epochs=0, device=device_worker 
        )
        ...
        adv_agent = PPOHybrid(
            ...
            critic=adv_dummy_critic, actor_lr=0, critic_lr=0, lmbda=0, eps=0, gamma=0, epochs=0, device=device_worker
        )
...
def run_MLP_simulation(
    ...
    eps=0.2,
    k_entropy=None,
    alpha_il=1.0, # 这里的 alpha_il 将只作为初始参考
    il_batch_size=128,
    il_batch_size2=128,
    ...
    teacher_actor.eval() 
    for param in teacher_actor.parameters(): param.requires_grad = False
    
    teacher_critic = ValueNet(state_dim, hidden_dim).to(device)
    teacher_critic.eval()
    for param in teacher_critic.parameters(): param.requires_grad = False
    ...
    for epoch in range(IL_epoches): 
        ...
        if epoch % 10 == 0:
            print(f"IL Epoch {epoch}: Loss {avg_actor_loss:.4f}")
    ...
    while True: 
        while total_steps < current_max_steps:
            ...
            # ==================================================================
            if global_batch_idx % save_interval == 0:
                best_member = max(population, key=lambda m: m.elo)
                
                actor_key = f"actor_rein{global_batch_idx}_P{best_member.id}"
                critic_key = f"critic_rein{global_batch_idx}_P{best_member.id}"
