...
def run_MLP_simulation(
    ...
    # [TD3] 按固定环境步数触发下一次更新（替代按 batch/回合触发）
    next_update_step_trigger = int(total_steps + TD3_update_step_interval)
    ...
    while True:
        while total_steps < current_max_steps:
            ...
            # [TD3] 触发条件：按固定环境步数触发更新（替代原来的按 batch/回合触发），
            # 避免间隔步数过多导致过拟合；每次更新最多执行 TD3_max_updates_per_batch 次梯度更新，
            # 且按顺序串行执行，不并行。
            if total_steps >= next_update_step_trigger:# and \
                ...
                # [TD3] 限制每次触发最多执行 TD3_max_updates_per_batch 次梯度更新，防止过拟合
                num_TD3_updates = min(num_TD3_updates, int(TD3_max_updates_per_batch))
                steps_since_update = 0
