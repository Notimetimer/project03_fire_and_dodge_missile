...
def run_rules(gamma=0.995, weight_reward=np.array([1,0,0]), action_cycle_multiplier=10, shielded=1, current_rule=2):
    gamma = gamma
        ...
        for _ in range(5):
            ...
            for i_episode in range(6):
                ...
                for count in range(round(args.max_episode_len / dt_maneuver)):
                    ...
                    done, b_reward_event, b_reward_constraint, b_reward_shaping = env.combat_terminate_and_reward('b', b_action_label, b_fire, end_reward_rate=0.1)
