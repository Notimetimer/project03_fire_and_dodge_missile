...
def worker_process(rank, pipe, args, state_dim, hidden_dim, 
    ...
    try:  # <--- 【新增】添加此行，并将下方所有代码整体缩进
        ...
        local_goo_actor = PolicyNetHybrid(state_dim, hidden_dim, action_dims_dict_goo_w).to(device_worker)
        local_goo_agent = HybridActorWrapper(local_goo_actor, action_dims_dict_goo_w, None, device_worker).to(device_worker)

        # 初始化对手网络 (对手使用完整action_dims_dict，已存储为双agent格式)
        adv_mav_actor = PolicyNetHybrid(state_dim, hidden_dim, action_dims_dict_mav_w).to(device_worker)
        adv_mav_agent = HybridActorWrapper(adv_mav_actor, action_dims_dict_mav_w, None, device_worker).to(device_worker)
        adv_goo_actor = PolicyNetHybrid(state_dim, hidden_dim, action_dims_dict_goo_w).to(device_worker)
        adv_goo_agent = HybridActorWrapper(adv_goo_actor, action_dims_dict_goo_w, None, device_worker).to(device_worker)
        ...
        while True:
            ...
            if cmd == 'RUN_EPISODE':
                ...
                # A. 同步权重 (极快) - 分别加载maverick和goose
                local_mav_agent.load_state_dict(actor_weights['maverick'])
                local_goo_agent.load_state_dict(actor_weights['goose'])
                ...
                if adv_is_rule:
                    ...
                    adv_mav_agent.load_state_dict(opp_data['maverick'])
                    adv_goo_agent.load_state_dict(opp_data['goose'])

                # C. 准备本回合容器
                # Worker 收集完整的 ego_trans (用于 SIL) 和 enm_trans (用于 SIL)
                # local_trans_mav 用于 maverick PPO 更新 (cat动作，reward=[0,1,0])
                ...
                for count in range(max_counts):
                    ...
                    if steps_run % action_cycle_multiplier == 0:
                        ...
                        # 2.3 产生新动作 (No Grad)
                        with torch.no_grad():
                            # Blue Decision: maverick负责cat，goose负责bern
                            b_state_check = env.unscale_state(b_check_obs)
                            b_mav_exec, _, _, _ = local_mav_agent.get_action(b_obs, explore=1, mask_on=fire_mask)
                            b_goo_exec, _, _, _ = local_goo_agent.get_action(b_obs, explore=1, mask_on=0)
                            b_action_exec = {'cat': b_mav_exec['cat'], 'bern': b_goo_exec['bern']}
                            ...
                            if adv_is_rule:
                                ...
                                r_mav_exec, _, _, _ = adv_mav_agent.get_action(r_obs, explore={'cont':0, 'cat':adv_explore, 'bern':1},
                                                        mask_on=fire_mask, temperature={'cat':opp_temperature, 'bern':1.0})
                                r_goo_exec, _, _, _ = adv_goo_agent.get_action(r_obs, explore={'cont':0, 'cat':0, 'bern':1},
                                                        mask_on=0, temperature={'cat':opp_temperature, 'bern':1.0})
                                r_action_exec = {'cat': r_mav_exec['cat'], 'bern': r_goo_exec['bern']}
...
def run_MLP_simulation(
    ...
    fly_act_dim = dummy_env.fly_act_dim
    fire_dim = dummy_env.fire_dim
    action_dims_dict = {'cont': 0, 'cat': fly_act_dim, 'bern': fire_dim}
    del dummy_env
    ...
    mav_actor_wrapper = HybridActorWrapper(mav_actor_net, action_dims_dict_mav, None, device).to(device)
    maverick = PPOHybrid(
        actor=mav_actor_wrapper,
        critic=mav_critic_net,
        actor_lr=actor_lr,
        critic_lr=critic_lr,
        lmbda=lmbda,
        epochs=epochs,
    ...
    goo_actor_net = PolicyNetHybrid(state_dim, hidden_dim, action_dims_dict_goo).to(device)
    goo_critic_net = ValueNet(state_dim, hidden_dim).to(device)
    goo_actor_wrapper = HybridActorWrapper(goo_actor_net, action_dims_dict_goo, None, device).to(device)
    goose = PPOHybrid(
        actor=goo_actor_wrapper,
        critic=goo_critic_net,
        actor_lr=actor_lr,
        critic_lr=critic_lr,
        lmbda=lmbda,
        epochs=epochs,
    ...
    save_meta_once(actor_meta_path, maverick.actor.state_dict())
    save_meta_once(critic_meta_path, maverick.critic.state_dict())
    ...
    if resume_dir is not None and os.path.exists(resume_dir):
        ...
        def _load_actor_ckpt(path):
            ckpt = torch.load(path, map_location=device)
            if isinstance(ckpt, dict) and 'maverick' in ckpt:
                maverick.actor.load_state_dict(ckpt['maverick'])
                goose.actor.load_state_dict(ckpt['goose'])
            else:
                maverick.actor.load_state_dict(ckpt)
                print("[Warning] Loaded single-agent ckpt into maverick only.")

        if os.path.exists(current_actor_path):
            _load_actor_ckpt(current_actor_path)
            ...
            if len(actor_files) > 0:
                ...
                _load_actor_ckpt(latest_actor)
                print(f"Loaded actor from: {latest_actor}")
        
        critic_path = os.path.join(log_dir, "critic.pt")
        if os.path.exists(critic_path):
            mav_critic_sd = torch.load(critic_path, map_location=device)
            maverick.critic.load_state_dict(mav_critic_sd)
            goose.critic.load_state_dict(mav_critic_sd)
        ...
        if os.path.exists(opt_path):
            try:
                ...
                maverick.actor_optimizer.load_state_dict(opt_states['actor_optimizer'])
                maverick.critic_optimizer.load_state_dict(opt_states['critic_optimizer'])
    ...
    try:
        torch.onnx.export(
            maverick.actor.net,
            dummy_state,
            os.path.join(log_dir, "maverick_actor.onnx"),
            export_params=True, opset_version=11, do_constant_folding=True,
            input_names=['state'], output_names=['cat_output']
        )
        print(f"Maverick Actor ONNX exported.")
    except Exception as e:
        print(f"Error exporting Maverick Actor ONNX: {e}")
    try:
        torch.onnx.export(
            goose.actor.net,
            dummy_state,
            os.path.join(log_dir, "goose_actor.onnx"),
            export_params=True, opset_version=11, do_constant_folding=True,
            input_names=['state'], output_names=['bern_output']
        )
        print(f"Goose Actor ONNX exported.")
    except Exception as e:
        print(f"Error exporting Goose Actor ONNX: {e}")
    try:
        torch.onnx.export(
            maverick.critic,
            dummy_state,
            os.path.join(log_dir, "student_critic.onnx"),
            export_params=True, opset_version=11, do_constant_folding=True,
            input_names=['state'], output_names=['value_estimate']
        )
        print(f"Critic ONNX exported.")
    except Exception as e:
    ...
    maverick.set_learning_rate(actor_lr=actor_lr_init_il, critic_lr=critic_lr_init_il)
    goose.set_learning_rate(actor_lr=actor_lr_init_il, critic_lr=critic_lr_init_il)
    ...
    for epoch in range(IL_epoches): 
        # maverick训练cat
        avg_actor_loss_mav, avg_critic_loss_mav, c_mav = maverick.MARWIL_update(
            original_il_transition_dict, 
            beta=beta_mixed, 
            batch_size=il_batch_size, # 显存如果够大可以适当调大
            label_smoothing=label_smoothing,
            action_heads_mask={'cont': False, 'cat': True, 'bern': False}, # maverick只训练cat
        )
        
        # goose训练bern
        avg_actor_loss_goo, avg_critic_loss_goo, c_goo = goose.MARWIL_update(
            original_il_transition_dict, 
            beta=beta_mixed, 
            batch_size=il_batch_size, # 显存如果够大可以适当调大
            label_smoothing=label_smoothing,
        ...
        if epoch % 1 == 0:
            # maverick指标
            logger.add("il_train/mav_avg_actor_loss", avg_actor_loss_mav, epoch)
            logger.add("il_train/mav_avg_critic_loss", avg_critic_loss_mav, epoch)
            # goose指标
            logger.add("il_train/goo_avg_actor_loss", avg_actor_loss_goo, epoch)
            logger.add("il_train/goo_avg_critic_loss", avg_critic_loss_goo, epoch)

            # [新增] MARWIL 监控：每个动作头独立的 NLL 与策略熵 (全采样固定 batch, no_grad)
            for _name, _val in [
                ("il_train_mav/nll_cont", getattr(maverick, "marwil_nll_cont", None)),
                ("il_train_mav/nll_cat", getattr(maverick, "marwil_nll_cat", None)),
                ("il_train_mav/entropy_cont", getattr(maverick, "marwil_entropy_cont", None)),
                ("il_train_mav/entropy_cat", getattr(maverick, "marwil_entropy_cat", None)),
                ("il_train_mav/accuracy_cont", getattr(maverick, "marwil_accuracy_cont", None)),
                ("il_train_mav/accuracy_cat", getattr(maverick, "marwil_accuracy_cat", None)),
                ("il_train_mav/weight_mean", getattr(maverick, "marwil_weight_mean", None)),
                ("il_train_mav/weight_clip_frac", getattr(maverick, "marwil_weight_clip_frac", None)),
                ("il_train_mav/adv_std", getattr(maverick, "marwil_adv_std", None)),
                ("il_train_mav/adv_max", getattr(maverick, "marwil_adv_max", None)),
                ("il_train_mav/adv_mean", getattr(maverick, "marwil_adv_mean", None)),
                ("il_train_mav/adv_positive_frac", getattr(maverick, "marwil_adv_positive_frac", None)),
                # goose指标
                ("il_train_goo/nll_bern", getattr(goose, "marwil_nll_bern", None)),
                ("il_train_goo/entropy_bern", getattr(goose, "marwil_entropy_bern", None)),
                ("il_train_goo/accuracy_bern", getattr(goose, "marwil_accuracy_bern", None)),
                ("il_train_goo/weight_mean", getattr(goose, "marwil_weight_mean", None)),
                ("il_train_goo/weight_clip_frac", getattr(goose, "marwil_weight_clip_frac", None)),
                ("il_train_goo/adv_std", getattr(goose, "marwil_adv_std", None)),
                ("il_train_goo/adv_max", getattr(goose, "marwil_adv_max", None)),
                ("il_train_goo/adv_mean", getattr(goose, "marwil_adv_mean", None)),
                ("il_train_goo/adv_positive_frac", getattr(goose, "marwil_adv_positive_frac", None)),
            ]:
                if _val is not None:
                    logger.add(_name, _val, epoch)

            print(f"Epoch {epoch}: MAV Actor Loss: {avg_actor_loss_mav:.4f}, Critic Loss: {avg_critic_loss_mav:.4f} | GOO Actor Loss: {avg_actor_loss_goo:.4f}, Critic Loss: {avg_critic_loss_goo:.4f}")
    
    # MARWIL 结束后恢复 bern bias，防止稀疏开火动作被拉向高熵中间态（无济于事）
    if hasattr(goose.actor.net, 'fc_bern'):
        with torch.no_grad():
            goose.actor.net.fc_bern[-1].bias.clamp_(max=-2.5)
    ...
    # 存储在线训练前的网络参数，maverick和goose合并到同一文件
    int_agent_name = "actor_rein0"
    torch.save({'maverick': maverick.actor.state_dict(), 'goose': goose.actor.state_dict()},
               os.path.join(log_dir, f"{int_agent_name}.pt"))
    ...
    if collape_recover["collapsed"]:
        ...
        maverick.reset_optimizer()
        goose.reset_optimizer()
    else:
    ...
    # 全局 Buffer (用于攒够 Batch 训练) - maverick和goose各一个
    ...
    while True:
        while total_steps < current_max_steps:
            ...
            if total_steps >= trigger:
                ...
                current_weights = {'maverick': {k: v.cpu().clone() for k, v in maverick.actor.state_dict().items()},
                                   'goose': {k: v.cpu().clone() for k, v in goose.actor.state_dict().items()}}
            ...
            # A. 获取当前策略权重 (CPU) - 合并maverick和goose
            current_actor_weights = {'maverick': {k: v.cpu() for k, v in maverick.actor.state_dict().items()},
                                     'goose': {k: v.cpu() for k, v in goose.actor.state_dict().items()}}
            ...
            for res in batch_results:
                ...
                l_tr_mav = res['trans_mav']
                l_tr_goo = res['trans_goo']
                for k in transition_dict_mav:
                    transition_dict_mav[k].extend(l_tr_mav[k])
            ...
            if batch_idx % save_interval == 0 and \
                len(transition_dict_mav['dones']) >= transition_dict_threshold:
                # # --- 4. 执行训练 (PPO Update) ---
                # # 当收集的数据量超过 capacity 时更新
                # if len(transition_dict_mav['dones']) >= transition_dict_threshold:
                ...
                current_ppo_grad = maverick.pre_clip_actor_grad
                ...
                alpha_il_real = alpha_il
                
                # 记录 Log
                logger.add("mav/actor_loss", maverick.actor_loss, total_steps)
                logger.add("mav/critic_loss", maverick.critic_loss, total_steps)
                logger.add("mav/entropy_cat", maverick.entropy_cat, total_steps)
                logger.add("goo/actor_loss", goose.actor_loss, total_steps)
                logger.add("goo/critic_loss", goose.critic_loss, total_steps)
                logger.add("goo/entropy_bern", goose.entropy_bern, total_steps)
                logger.add("goo/max_fire_prob", goose.max_fire_prob, total_steps)
                logger.add("goo/min_fire_prob", goose.min_fire_prob, total_steps)
                ...
                # 正常保存模型：maverick和goose合并到同一文件，用key区分
                combined_actor_sd = {'maverick': maverick.actor.state_dict(), 'goose': goose.actor.state_dict()}
                torch.save(combined_actor_sd, os.path.join(log_dir, f"{actor_key}.pt"))
                torch.save(combined_actor_sd, os.path.join(log_dir, "current_actor.pt"))
                torch.save(maverick.critic.state_dict(), os.path.join(log_dir, "critic.pt"))
                ...
                # --- 例行保存优化器状态和步数 ---
                torch.save({
                    'actor_optimizer': maverick.actor_optimizer.state_dict(),
                    'critic_optimizer': maverick.critic_optimizer.state_dict(),
                    'rnd_target': maverick.rnd_target.state_dict() if maverick.rnd_target is not None else None,
                    'rnd_prediction': maverick.rnd_prediction.state_dict() if maverick.rnd_prediction is not None else None,
                    'rnd_optimizer': maverick.rnd_optimizer.state_dict() if maverick.rnd_optimizer is not None else None,
