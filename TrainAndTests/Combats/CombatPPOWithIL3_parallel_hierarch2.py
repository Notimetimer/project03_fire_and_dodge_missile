...
from Algorithms.PPOHybrid_ADistill2 import PPOHybrid, PolicyNetHybrid, HybridActorWrapper
        ...
        while True:
            ...
            if cmd == 'RUN_EPISODE':
                ...
                for count in range(max_counts):
                    ...
                    if steps_run % action_cycle_multiplier == 0:
                        ...
                        if steps_run > 0:
                            ...
                            append_experience(local_trans, last_decision_obs, last_decision_state, current_action, reward_for_learn, global_state, False, not dead_dict['b'])
                            append_experience(ego_trans, last_decision_obs, last_decision_state, current_action_exec, reward_for_learn, global_state, False, not dead_dict['b'])
                            append_experience(enm_trans, last_enm_decision_obs, last_decision_state, current_enm_action_exec, reward_for_enm, global_state, False, not dead_dict['r'])

                        # 2.2 更新上一帧记录
                        last_decision_obs = b_obs
                        last_decision_state = global_state
                        last_enm_decision_obs = r_obs
                        last_enm_decision_state = global_state
                ...
                if last_decision_state is not None:
                    ...
                    append_experience(local_trans, last_decision_obs, last_decision_state, current_action, reward_for_learn, next_global_state, True, not dead_dict['b'])
                    append_experience(ego_trans, last_decision_obs, last_decision_state, current_action_exec, reward_for_learn, next_global_state, True, not dead_dict['b'])
                    append_experience(enm_trans, last_enm_decision_obs, last_enm_decision_state, current_enm_action_exec, reward_for_enm, next_global_state, True, not dead_dict['r'])
...
def run_MLP_simulation(
    ...
    # 3. 创建神经网络
    # [MAPPO] Critic 输入为红蓝双方全局状态的拼接（先红后蓝），维度 2*state_dim
    ...
    for epoch in range(IL_epoches): 
        avg_actor_loss, avg_critic_loss, c = student_agent.MARWIL_update(
            il_dict_ma if il_dict_ma is not None else original_il_transition_dict, 
            beta=beta_mixed, 
    ...
    if IL_epoches + use_sil > 0:  # 只要出现模仿学习就得准备好初始的模仿池
        ...
        if os.path.exists(il_buffer_path):
            ...
            original_data_input = il_dict_ma if il_dict_ma is not None else (original_il_transition_dict0 if use_init_data else None)
    ...
    while True:
        while total_steps < current_max_steps:
            ...
            if batch_idx % save_interval == 0 and \
                ...
                # 只蒸馏机动(cat)，不蒸馏开火(bern)，固定teacher为rule6
                if use_ADistill:
