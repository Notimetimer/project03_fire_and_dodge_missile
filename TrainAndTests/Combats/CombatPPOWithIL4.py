...
def run_MLP_simulation(
    ...
    # 1. 初始 Elo 表构建 (确保是 dict 结构)
    elo_ratings = {}
    for k, v in init_elo_ratings.items():
        # 初始化时默认为 0.5 胜率
        elo_ratings[k] = {'elo': v, 'win_rate': 0.5}
    # 2. 尝试加载已有的 ELO 记录
    if os.path.exists(elo_json_path):
        with open(elo_json_path, 'r', encoding='utf-8') as f:
            ...
            for k, v in loaded_data.items():
                if k == "__CURRENT_MAIN__": # 特殊字段直接复制
                    continue
                
                if isinstance(v, (float, int)):
                    # 旧格式兼容: {name: 1200.0} -> {name: {'elo': 1200.0, 'win_rate': 0.5}}
                    elo_ratings[k] = {'elo': float(v), 'win_rate': 0.5}
                elif isinstance(v, dict):
                    # 新格式直接读取，确保有默认值
                    elo_ratings[k] = {
                        'elo': v.get('elo', 1200.0), 
                        'win_rate': v.get('win_rate', 0.5)
                    }
            
            # 读取断点续传的 Main Elo
            if "__CURRENT_MAIN__" in loaded_data:
                # 兼容旧格式 float 和 新格式 dict
                main_val = loaded_data["__CURRENT_MAIN__"]
                if isinstance(main_val, dict):
                    main_agent_elo = main_val.get('elo', 1200.0)
                else:
                    main_agent_elo = float(main_val)
    else:
        main_agent_elo = 1200.0
    ...
    if (not elo_ratings) or IL_epoches>0:
        ...
        if not self_play_type=='None':
            elo_ratings[init_opponent_name] = {'elo':1200, 'win_rate':0.5}
    ...
    def get_opponent_probabilities(elo_ratings, target_elo=None, sigma=400, SP_type='PFSP_balanced'):
        """
        基于胜率的概率匹配。
        - elo_ratings: dict, 格式为 {name: {'elo': 1200, 'win_rate': 0.5}}
        - target_elo: (已废弃，保留接口) 在新的胜率逻辑中，中心点由公式固定为 0.5。
        - sigma: 控制分布的宽度。
        - SP_type: 'PFSP_balanced', 'PFSP_challenge', 'FSP', 'SP', 'None'
        """
        keys = list(elo_ratings.keys())
        if len(keys) == 0:
            return np.array([]), []

        # --- 逻辑 1: PFSP (基于胜率的 Logit 高斯分布) ---
        if SP_type.startswith('PFSP'):
            # 提取胜率，如果旧数据没有win_rate，默认为0.5
            win_rates = []
            for k in keys:
                val = elo_ratings[k]
                if isinstance(val, dict):
                    # 获取胜率，如果没有则默认为 0.5
                    win_rates.append(val.get('win_rate', 0.5))
                else:
                    # 如果数据是旧格式 (float/int)，默认为 0.5
                    win_rates.append(0.5)
            
            x = np.array(win_rates, dtype=np.float64)
            
            # 数值稳定性截断，防止 log(0) 或 log(inf)
            # 胜率限制在 [0.01, 0.99] 之间
            x = np.clip(x, 0.001, 0.999)
            if SP_type == 'PFSP_balanced' or 'PFSP':
                scores = np.exp(-0.5*(400/sigma*np.log10(1/x-1))**2)
            elif SP_type == 'PFSP_challenge':
                scores = np.exp(-0.5*(400/sigma*(1.0-x))**2)
    ...
    while total_steps < int(max_steps):
        ...
        # 1. 计算当前排位 rank_pos
        valid_elo_values = [v['elo'] for k, v in elo_ratings.items() if not k.startswith("__")]
        ...
        # if "Rule_0" not in elo_ratings and (not pure_self_play):
        #     elo_ratings["Rule_0"] = 1200
            
        # if rank_pos < 0.4 and (not pure_self_play):
        ...
        if "Rule" in selected_opponent_name:
            ...
            if os.path.exists(adv_path):
                ...
                print(f"Eps {i_episode}: Opponent Loaded from {selected_opponent_name} (ELO: {elo_ratings[selected_opponent_name]['elo']:.0f})")
            else:
        ...
        if selected_opponent_name in elo_ratings and not is_kicked_opponent:
            # 获取当前对手数据对象
            opponent_stats = elo_ratings[selected_opponent_name]
            
            # --- A. 更新 Win Rate (EMA) ---
            # 定义: win_rate 是"对手对当前智能体的胜率"
            # 如果 Current_Result (actual_score) 为 1 (主智能体赢), 则对手输, 对手得分为 0
            # 如果 Current_Result (actual_score) 为 0 (主智能体输), 则对手赢, 对手得分为 1
            opponent_win_score = 1.0 - actual_score
            
            old_wr = opponent_stats.get('win_rate', 0.5)
            # EMA 更新: 0.9 * Old + 0.1 * Current_Result
            new_wr = 0.9 * old_wr + 0.1 * opponent_win_score
            
            opponent_stats['win_rate'] = new_wr # 写回
            
            # --- B. 更新 ELO (仅作为记录或辅助) ---
            adv_elo = opponent_stats['elo']
            ...
            elo_ratings[selected_opponent_name] = opponent_stats
            
            print(f"  Result: Score={actual_score}, WR_Adv: {old_wr:.3f}->{new_wr:.3f}")
            print(f"  Elo: Main {prev_main_elo:.0f}->{main_agent_elo:.0f}, Adv {adv_elo:.0f}->{new_adv_elo:.0f}")
        ...
        if i_episode % 5 == 0:
            ...
            try:
                ...
                with open(tmp_path, "w", encoding="utf-8") as f:
                    ...
                    save_data["__CURRENT_MAIN__"] = {'elo': main_agent_elo, 'win_rate': 0.5}
            ...
            # 过滤掉特殊 Key
            valid_elos = {k: v['elo'] for k, v in elo_ratings.items() if not k.startswith("__") and isinstance(v, dict)}
            # 顺便记录 Win Rate 分布
            valid_wrs = {k: v['win_rate'] for k, v in elo_ratings.items() if not k.startswith("__") and isinstance(v, dict)}
            if valid_elos:
                ...
                if selected_opponent_name in valid_wrs:
                    logger.add("Elo/Opponent_Win_Rate", valid_wrs[selected_opponent_name], total_steps)
