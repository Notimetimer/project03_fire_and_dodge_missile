...
def basic_rules(state_check, rules_num, last_action=0, p_random=0):
    ...
    speed_T = state_check["target_information"][5]
    ...
    # 1. 计算初始的开火意图：保留原有几何门限与 t_fired，
    #    仅将 (distance>12e3 and abs(AA_hor)<45°) 这条 guard 用攻击区命中概率>0.5 代替
    fire_missile = False
    attack_zone_ok = None

    def _attack_zone_good():
        nonlocal attack_zone_ok
        if attack_zone_ok is None:
            cos_theta = state_check["ego_main"][2]
            theta = atan2(sin_theta, cos_theta)

            p_carrier_ = np.array([0.0, alt, 0.0])
            v_carrier_ = np.array([speed * cos_theta, speed * sin_theta, 0.0])

            p_target_ = np.array([
                distance * cos_delta_psi * cos(delta_theta + theta),
                alt + distance * sin(delta_theta + theta),
                distance * sin_delta_psi * cos(delta_theta + theta)
            ])
            v_target_ = np.array([
                speed_T * cos(delta_psi + AA_hor),
                0.0,
                speed_T * sin(delta_psi + AA_hor)
            ])

            try:
                attack_zone_ok = hit_prob(p_carrier_, v_carrier_, p_target_, v_target_) > 0.5
            except Exception:
                attack_zone_ok = False
        return attack_zone_ok
    ...
    if (case1 or case2) and ATA < 60 * pi/180 and abs(delta_psi) < 30*pi/180:
        if t_fired >= 40 and not on_guiding and ammo > 0.5 and _attack_zone_good():
            fire_missile = True
    ...
    if rules_num == 0:
        ...
        if (distance < 95e3) and ATA < 60 * pi/180 and abs(delta_psi) < 30*pi/180:
            if t_fired >= 10 and not on_guiding and ammo > 0.5 and _attack_zone_good():
                fire_missile = True
...
if __name__=='__main__':
    ...
    try:
        ...
        # 采集不同轨迹的动作
        for i_episode in [3,5]: # range(6): # 5
