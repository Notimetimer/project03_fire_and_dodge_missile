...
# 定义 PBT 的变异空间
PBT_HYPERPARAMS = {
    'lr_mutation': [0.8, 1.2],       # 学习率放大或缩小
    'sp_types': ['PFSP_balanced', 'PFSP_challenge', 'SP', 'PFSP_balanced'], # PFSP 变异池
    'pbt_interval': 50000            # 每多少步触发一次 PBT
}

def pbt_exploit_and_explore(worker_id, agent, current_score, shared_dict, log_dir, device):
    """
    PBT 核心逻辑：淘汰与变异
    """
    # 1. 更新自己的成绩到共享记分板
    # shared_dict['scoreboard'] 是一个 Dict: {worker_id: {'score': float, 'lr': float, 'sp_type': str}}
    scoreboard = shared_dict['scoreboard']
    scoreboard[worker_id] = {
        'score': current_score,
        'lr': agent.actor_optimizer.param_groups[0]['lr'],
        'sp_type': agent.custom_sp_type # 假设我们在 agent 或 worker 里存了这个变量
    }
    shared_dict['scoreboard'] = scoreboard # 触发 Manager 同步更新

    # 2. 获取当前最强和最弱
    all_scores = [(k, v['score']) for k, v in scoreboard.items()]
    all_scores.sort(key=lambda x: x[1], reverse=True) # 降序，第一个是最好的
    
    best_worker_id, best_score = all_scores[0]
    worst_worker_id, worst_score = all_scores[-1]
    
    # 3. 判断我是不是最弱的 (且差距足够大)
    if worker_id == worst_worker_id and (best_score - current_score > 0.05):
        print(f"\n[PBT] Worker {worker_id} (Score {current_score:.2f}) is ELIMINATED by Worker {best_worker_id} (Score {best_score:.2f})")
        
        # --- A. Exploit (覆盖权重) ---
        # 从磁盘加载 Best Worker 的最新权重 (Worker 必须定期保存自己的 latest.pt)
        best_model_path = os.path.join(log_dir, f"worker_{best_worker_id}_latest.pt")
        if os.path.exists(best_model_path):
            agent.actor.load_state_dict(torch.load(best_model_path, map_location=device))
            # 也可以选择性加载 Critic，或者 Critic 不加载让他自己适应
        ...
        # --- B. Explore (变异超参) ---
        best_config = scoreboard[best_worker_id]
        
        # 1. 变异学习率
        new_lr = best_config['lr'] * np.random.choice(PBT_HYPERPARAMS['lr_mutation'])
        new_lr = np.clip(new_lr, 1e-6, 1e-3) # 保护一下
        agent.set_learning_rate(actor_lr=new_lr, critic_lr=new_lr * 5.0) # 保持 actor/critic 比例
        
        # 2. 变异 PFSP 策略
        # 有一定概率随机换一个，或者继承强者的
        if np.random.rand() < 0.3:
            new_sp_type = np.random.choice(PBT_HYPERPARAMS['sp_types'])
        else:
            new_sp_type = best_config['sp_type']
        
        print(f"      -> New LR: {new_lr:.6f}, New SP_Type: {new_sp_type}")
        return new_sp_type # 返回新的策略类型供外部循环使用
