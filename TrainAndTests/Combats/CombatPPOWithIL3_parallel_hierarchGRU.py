...
from VsBaseline_while_training_hierarch_plusGRU import test_worker
...
def restructure_actions(actions_data):
    ...
    if isinstance(actions_data, list) and len(actions_data) > 0:
        ...
        # 转换为 Numpy Array 并调整形状为 (Batch, N_Heads)
        # PPOHybrid2 里的 expert_cat[:, i] 需要 expert_cat 是二维的
        
        # 1. 'cat': 离散动作，转为 int64。单头 reshape 为 (N, 1)，多头保持 (N, N_Heads)
...
def append_experience(td, obs, state, action, reward, next_state, done, active_mask, actor_h=None, critic_h=None):
    """
    统一把一次蓝方经验追加到 transition_dict。
    修改：增加 obs 输入，用于存储局部观测；增加 actor_h/critic_h 用于 GRU 训练。
    """
    ...
    if 'actor_h' in td:
        td['actor_h'].append(actor_h)
    if 'critic_h' in td:
        td['critic_h'].append(critic_h)
    return td
    ...
    try:  # <--- 【新增】添加此行，并将下方所有代码整体缩进
        ...
        while True:
            ...
            if cmd == 'RUN_EPISODE':
                ...
                # C. 准备本回合容器
                # local_trans 用于 PPO 更新 (只包含 Blue 视角)，并记录 GRU 初始隐状态
                local_trans = {'obs': [], 'states': [], 'actions': [], 'next_states': [], 'rewards': [], 'dones': [], 'active_masks': [], 'actor_h': []}
                ...
                for count in range(max_counts):
                    ...
                    if steps_run % action_cycle_multiplier == 0:
                        ...
                        if steps_run > 0:
                            ...
                            append_experience(local_trans, last_decision_obs, last_decision_state, current_action, reward_for_learn, b_state_global, False, not dead_dict['b'], actor_h=current_action_actor_h)
                            append_experience(ego_trans, last_decision_obs, last_decision_state, current_action_exec, reward_for_learn, b_state_global, False, not dead_dict['b'], actor_h=current_action_actor_h)
                        ...
                        # 2.3 产生新动作 (No Grad)，并记录用于当前动作的 GRU 隐状态
                        with torch.no_grad():
                            # Blue Decision
                            b_state_check = env.unscale_state(b_check_obs)
                            b_h_in = local_actor_h
                            b_action_exec, _, b_h_out, _ = local_agent.get_action(b_obs, h=b_h_in, explore=1, mask_on=fire_mask)
                            # 记录产生当前动作所使用的隐状态
                            if b_h_in is None:
                                L_a, H_a = b_h_out.shape[0], b_h_out.shape[2]
                                current_action_actor_h = np.zeros((L_a, H_a), dtype=np.float32)
                            else:
                                current_action_actor_h = b_h_in
                            local_actor_h = b_h_out[:, 0, :].detach().cpu().numpy()
                            ...
                            if adv_is_rule:
                                ...
                                r_action_exec, _, r_h_out, _ = adv_agent.get_action(r_obs, h=adv_actor_h, explore={'cont':0, 'cat':adv_explore, 'bern':1}, 
                                                        mask_on=fire_mask, temperature={'cat':opp_temperature, 'bern':1.0})
                                adv_actor_h = r_h_out[:, 0, :].detach().cpu().numpy() if r_h_out is not None else None
                ...
                if last_decision_state is not None:
                    append_experience(local_trans, last_decision_obs, last_decision_state, current_action, reward_for_learn, next_b_state_global, True, not dead_dict['b'], actor_h=current_action_actor_h)
                    append_experience(ego_trans, last_decision_obs, last_decision_state, current_action_exec, reward_for_learn, next_b_state_global, True, not dead_dict['b'], actor_h=current_action_actor_h)
...
def run_MLP_simulation(
    ...
    # 全局 Buffer (用于攒够 Batch 训练)，追加 actor_h 以支持 GRU 训练
    empty_transition_dict = {'obs': [], 'states': [], 'actions': [], 'next_states': [], 'rewards': [], 'dones': [], 'active_masks': [], 'actor_h': []}
    ...
    while True:
        while total_steps < current_max_steps:
            ...
            if batch_idx % save_interval == 0 and \
                ...
                # 将经验切分为 5-step 序列，并生成 init_h_actor/init_h_critic
                transition_dict = student_agent.reshape_for_rnn(transition_dict, seq_len=5)
