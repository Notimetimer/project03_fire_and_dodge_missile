...
from Algorithms.PPOHybridClockWise import PolicyNetHybrid, HybridActorWrapper # 纯MLP
...
if __name__ == "__main__":
    ...
    action_dims_dict = {'cont': 0, 'lin': 1, 'circ': 1, 'bern': env.fire_dim}
    ...
    actor_wrapper = HybridActorWrapper(actor_net, action_dims_dict, None, device, circ_angles=[0, np.pi/3, np.pi/2, np.pi, -np.pi/2, -np.pi/3], lin_angles=[np.pi/4, np.pi/8, 0.0, -np.pi/8, -np.pi/2]).to(device)
