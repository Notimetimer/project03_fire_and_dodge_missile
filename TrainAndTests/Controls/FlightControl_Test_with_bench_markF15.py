...
from TrainAndTests.Controls.FlightControl_Train_dual_a_outF15 import *
...
if mission_name != "PID":
    ...
    log_dir = os.path.join(pre_log_dir, "超控标准差F15-run-20260707-145507")

    # 用新函数加载 actor：若想强制加载编号为 990 的模型，传入 number=990
    actor_path = load_actor_from_log(log_dir, number=None) # 14200) # None
    if not actor_path:
...
for init_h in height_list:
    height_req = np.clip(init_h - 5000, 1000, 25000)
    for target_v in speed_list:
        ...
        while not done:
            ...
            if chasing_wave:
                ...
                env.height_req = np.clip(env.height_req, 1000, 25000)
            else:
                if env.t >= 30:
                    env.height_req = 5000
