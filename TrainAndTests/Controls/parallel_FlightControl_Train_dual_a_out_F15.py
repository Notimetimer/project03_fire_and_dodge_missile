...
from Envs.UAVmodel6dF15 import UAVModel
...
from TrainAndTests.Controls.FlightControl_Train_dual_a_outF15 import track_env
    ...
    try:
        ...
        while True:
            ...
            if cmd == 'RUN_EPISODE':
                ...
                # 使用传入的 warm_up 调整难度
                height_req = np.clip(init_height + 1 * np.random.uniform(-1, 1) * 5000, 1000, 20000)
                ...
                while not done:
                    ...
                    v_req += np.random.randn() * 5 * dt_decide # 速度目标也在动
                    
                    # # 每53s剧变
                    if env.t % 53 == 0:
                        height_req += np.random.randn() * 3000
                        psi_req += np.random.randn() * np.radians(120)
                        v_req += np.random.randn() * np.radians(50)

                    # 限制到允许范围
                    env.height_req = np.clip(height_req, 1000, 20000)
...
if __name__=='__main__':
    ...
    dt_move = 0.02
    k_entropy={'cont':0.01, 'cat':0.0, 'bern':0.0} # 0.01
