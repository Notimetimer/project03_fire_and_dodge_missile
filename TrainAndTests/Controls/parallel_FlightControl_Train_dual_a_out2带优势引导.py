...
if __name__=='__main__':
    ...
    alpha_distill_start = 0.5  # PID 优势蒸馏初始强度
    AFiltered = 1              # 1：只给 advantage>mean(advantage) 的样本加偏移
    ...
    try:
        ...
        while rl_steps < max_steps:
            ...
            # 计算蒸馏参数 (Master 使用)
            alpha_distill = alpha_distill_start * max(0.0, 1.0 - 0.4 * rl_steps / max_steps)
            distil_epochs = 0 # max(int(10 * (1 - 0.9 * warm_up)), 1)

            # 5. 模型更新
            agent.update(master_transition_dict, adv_normed=1, mini_batch_size=512,
                         alpha_distill=alpha_distill, teacher_actor=teacher_agent, AFiltered=AFiltered)
