...
from Envs.UAVmodel6dF15 import UAVModel  # UAVmodel6d
...
class track_env():
    def __init__(self, dt_move=0.02, tacview_show=0, time_limit=3*60):
        ...
        self.max_alt_safe =19e3
        self.max_alt = 20e3

        # Realistic flight limits
        self.min_alpha_air = -12 # 
        self.max_alpha_air = 20 # 18
        self.max_beta_air = 8
        self.min_ny = -3
        self.max_ny = 9
    ...
    def get_reward(self, action):
        ...
        # 高度限制奖励/惩罚，临时更改，在不安全高度往里没有奖励，往外跑有惩罚 2*
        r_alt += 4*((alt <= 3000) * np.clip(self.RUAV.vu / 100, -2, 0) + \
        ...
        # 俯冲的时候不能超过1Ma
        if theta < np.radians(-30):
            speed2req = min(speed2req, 0.8*340-speed) # 
        r_speed = self.RUAV.Nx * np.sign(speed2req) * 5  # 0.5 小了
        ...
        if alpha_air > self.max_alpha_air/2:
            reward_alpha -= 10 * abs((alpha_air)/self.max_alpha_air) # **2
        if alpha_air < self.min_alpha_air/2:
            reward_alpha -= 10 * abs((alpha_air)/self.min_alpha_air) # **2
        ny = self.RUAV.Ny
        # if ny<=-1:
        ...
        if ny > self.max_ny/2:
            reward_alpha -= 8 * abs((ny/self.max_ny)) # 10
        if ny < 0:
            reward_alpha -= 8 * abs((ny/self.min_ny)) # 10
        
        # 侧滑角惩罚（尽量少侧滑）
        # 原有写法
        # reward_beta = - 1.2 * abs(beta_air/5) # 1可能有些小，2可能大了
        # 平方项修改
        reward_beta = - 1.8 * abs(beta_air/self.max_beta_air)**2
        ...
        if self.tacview_show:
            ...
            if not self.RUAV.dead:
                ...
                data_to_send += (
                            ...
                            f"Name=F15,Pilot={pilot},Color={color}\n"
                        )
                # 绘制目标
