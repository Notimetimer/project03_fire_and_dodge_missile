...
plot_comparison(ax1_l, [df_ppo_steady, df_pid_steady, df_original_ppo_steady], 'psi', ['PPO', 'PID', 'Original PPO'], is_error=False, req_col='psi_req', unwrap=True)
ax1_l.set_ylabel("heading / degree"); ax1_l.set_title("Mild Maneuver"); ax1_l.legend(loc='lower right', draggable=True); ax1_l.grid(True)
ax1_l.yaxis.set_major_locator(ticker.MultipleLocator(45))

ax1_m = plt.subplot(3, 3, 2)
plot_comparison(ax1_m, [df_ppo_norm, df_pid_norm, df_original_ppo_norm], 'psi', ['PPO', 'PID', 'Original PPO'], is_error=False, req_col='psi_req', unwrap=True)
ax1_m.set_ylabel("heading / degree"); ax1_m.set_title("Violent Maneuver"); ax1_m.legend(loc='lower right', draggable=True); ax1_m.grid(True)
ax1_m.yaxis.set_major_locator(ticker.MultipleLocator(45))

ax1_r = plt.subplot(3, 3, 3)
plot_comparison(ax1_r, [df_ppo_splits, df_pid_splits, df_original_ppo_splits], 'psi', ['PPO', 'PID', 'Original PPO'], is_error=False, req_col='psi_req', unwrap=True)
ax1_r.set_ylabel("heading / degree"); ax1_r.set_title("Split-S Maneuver"); ax1_r.legend(loc='lower right', draggable=True); ax1_r.grid(True)
ax1_r.yaxis.set_major_locator(ticker.MultipleLocator(45))

# 2. 俯仰角对比
ax2_l = plt.subplot(3, 3, 4)
plot_comparison(ax2_l, [df_ppo_steady, df_pid_steady, df_original_ppo_steady], 'theta', ['PPO', 'PID', 'Original PPO'], req_col='theta_req')
ax2_l.set_ylabel("pitch angle / degree"); ax2_l.legend(loc='lower right', draggable=True); ax2_l.grid(True)
# ax2_l.yaxis.set_major_locator(ticker.MultipleLocator(45))

ax2_m = plt.subplot(3, 3, 5)
plot_comparison(ax2_m, [df_ppo_norm, df_pid_norm, df_original_ppo_norm], 'theta', ['PPO', 'PID', 'Original PPO'], req_col='theta_req')
ax2_m.set_ylabel("pitch angle / degree"); ax2_m.legend(loc='lower right', draggable=True); ax2_m.grid(True)
# ax2_l.yaxis.set_major_locator(ticker.MultipleLocator(45))

ax2_r = plt.subplot(3, 3, 6)
plot_comparison(ax2_r, [df_ppo_splits, df_pid_splits, df_original_ppo_splits], 'theta', ['PPO', 'PID', 'Original PPO'], req_col='theta_req')
ax2_r.set_ylabel("pitch angle / degree"); ax2_r.legend(loc='lower right', draggable=True); ax2_r.grid(True)
# ax2_l.yaxis.set_major_locator(ticker.MultipleLocator(45))

# 3. 速度对比
ax3_l = plt.subplot(3, 3, 7)
plot_comparison(ax3_l, [df_ppo_steady, df_pid_steady, df_original_ppo_steady], 'v', ['PPO', 'PID', 'Original PPO'], req_col='v_req')
ax3_l.set_ylabel("speed / m/s"); ax3_l.set_xlabel("Time / second"); ax3_l.legend(loc='lower right', draggable=True); ax3_l.grid(True)

ax3_m = plt.subplot(3, 3, 8)
plot_comparison(ax3_m, [df_ppo_norm, df_pid_norm, df_original_ppo_norm], 'v', ['PPO', 'PID', 'Original PPO'], req_col='v_req')
ax3_m.set_ylabel("speed / m/s"); ax3_m.set_xlabel("Time / second"); ax3_m.legend(loc='lower right', draggable=True); ax3_m.grid(True)

ax3_r = plt.subplot(3, 3, 9)
plot_comparison(ax3_r, [df_ppo_splits, df_pid_splits, df_original_ppo_splits], 'v', ['PPO', 'PID', 'Original PPO'], req_col='v_req')
ax3_r.set_ylabel("speed / m/s"); ax3_r.set_xlabel("Time / second"); ax3_r.legend(loc='lower right', draggable=True); ax3_r.grid(True)
...
# ax2_1.plot(df_original_ppo_norm['time'], df_original_ppo_norm['alpha'], 'lightgreen', linestyle='-.', label='Original PPO', alpha=0.6)
...
# ax2_2.plot(df_original_ppo_norm['time'], df_original_ppo_norm['phi'], 'lightgreen', linestyle='-.', label='Original PPO', alpha=0.6)
...
# ax2_5.set_ylabel("Normalized Command"); ax2_5.set_xlabel("Time / second")
...
scenarios = [
    ("Mild Maneuver", df_ppo_steady, df_pid_steady, df_original_ppo_steady),
    ("Violent Maneuver", df_ppo_norm, df_pid_norm, df_original_ppo_norm),
    ("Split-S Maneuver", df_ppo_splits, df_pid_splits, df_original_ppo_splits)
]
