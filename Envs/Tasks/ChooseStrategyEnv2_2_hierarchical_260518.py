...
class ChooseStrategyEnv(BaseChooseStrategyEnv):
    ...
    def combat_terminate_and_reward(self, side, action_label, action_shoot, action_cycle_multiplier=30):
        ...
        if len(alive_ally_missiles) == 0 and not warning:
            # 瞄准奖励
            r_shaping += cos(delta_psi) * reward_weights['angle_advantage'] * (1-ego.dead)
            # 爬高奖励
            r_shaping += 0.5 * (ego.alt/1e5) * reward_weights['height_advantage'] * (1-ego.dead)
            r_shaping += 0.5 * min(ego.theta/(pi/4), 1) * reward_weights['angle_advantage'] * (1-ego.dead)
        # crank引导
        if len(alive_ally_missiles) > 0 and not warning:
            # 开火后crank下高，误差惩罚改为“保持中制导条件下的奖励”
            r_shaping += 4 * (1 - abs(pi/3-abs(delta_psi))/(pi/3)) * reward_weights['angle_advantage'] * missile_in_mid_term * (1-ego.dead)
            r_shaping += 5 * (1 - abs(-pi/4 - ego.theta) / (pi/4)) * reward_weights['angle_advantage'] * missile_in_mid_term * (1-ego.dead)
        # 防御引导
        if warning:
            # 受到威胁应该三九线/置尾和下高
            if abs(delta_psi_threat) < pi/2:
                r_shaping += min(abs(delta_psi_threat), pi/2)/(pi/2) * reward_weights['angle_advantage'] * (1-ego.dead)
            r_shaping += (-ego.theta)/(pi/2) * reward_weights['angle_advantage'] * (1-ego.dead)
