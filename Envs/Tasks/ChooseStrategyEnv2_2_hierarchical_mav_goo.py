...
class ChooseStrategyEnv(BaseChooseStrategyEnv):
    ...
    def combat_terminate_and_reward(self, side, action_label, action_shoot, action_cycle_multiplier=30, 
        ...
        # # 迎角惩罚
        # r_maverick -= reward_weights['aoa_penalty'] * ((ego.alpha_air*180/pi > 15)*(ego.alpha_air*180/pi-15) + \
        #                                                  (ego.alpha_air*180/pi < -5)*(-5 - ego.alpha_air*180/pi))
        # # 俯仰角惩罚
        # r_maverick -= reward_weights['pitch_penalty'] * (abs(ego.theta)/pi*2)
        ...
        #     r_maverick += 0.0005 * (1 - ego.dead) # 0.001
        ...
        if shoot >= 1:
            ...
            r_goose -= 10 * fire_reward_weight * (1.0 +np.tanh(
                ...
                fire_inside_weight[2] * 8 * (-1 + np.exp(1*np.abs(sub_of_radian(delta_psi+ego.psi, ego.psi_v)) / np.pi)) + # 至关重要
        ...
        if enm.escape_once:
            if len(fire_inside_weight) > 5:
                r_goose -= 10 * (1-enm.dead) * fire_reward_weight * fire_inside_weight[5] # 10
            else:
                r_goose -= 10 * (1-enm.dead) * fire_reward_weight # 20 # 10
        ...
        if ego._last_enm_threat_dist > threat_distance_threshold1 and enm_states["threat"][3] <= threat_distance_threshold1:
            r_goose += 4 * fire_reward_weight * weight_temp  # 稀疏威胁奖励，导弹送进10km以内就给，便于跟开火惩罚换算                   
        if ego._last_enm_threat_dist > threat_distance_threshold2 and enm_states["threat"][3] <= threat_distance_threshold2:
            r_goose += 8 * fire_reward_weight * weight_temp  # 稀疏威胁奖励，导弹送进10km以内就给，便于跟开火惩罚换算
