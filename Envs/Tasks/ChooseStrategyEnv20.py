    ...
    def combat_terminate_and_reward(self, side, action_label, action_shoot):
        ...
        if self.lose:
            ...
            if self.out_range(ego) or ego.alt < self.min_alt:
                reward_event -= 0.5  # 【新增】，如果不是被击落而是撞地/出界，惩罚更重
        if self.draw:
            reward_event -= 0.5
        ...
        reward_assisted += ((alt <= self.min_alt_safe) * np.clip(ego.vu / 100, -1, 1) + \
                (alt >= self.max_alt_safe) * np.clip(-ego.vu / 100, -1, 1)) * 1

        # 靠近边界惩罚
        o002ego_ = np.array([ego.pos_[0], ego.pos_[2]]) # 北，东
        ego_vh_ = np.array([ego.vel_[0], ego.vel_[2]])
        if d_hor<=50e3:
            reward_assisted -= (1-d_hor/50e3) * np.dot(ego_vh_, o002ego_)/norm(o002ego_ + 1e-3)/340 * 0.2
        else:
            reward_assisted += 0.2 # 不退后就有赏
        ...
        if len(alive_ally_missiles) == 0 and ego.ammo > 0 and not warning:
            # 角度优势度
            reward_assisted += (ATA_enm / pi - alpha / pi) * 1
            # 高度优势度
            reward_assisted += (ego.alt-enm.alt)/5000 * 0.1                 
        ...
        if warning:
            ...
            if threat_distance<=24e3:
                reward_assisted += 0.5 * abs(delta_psi_threat) / pi
                reward_assisted += np.dot(ego.vel_,threat_directio_n)/340 * 0.5
            else:
                reward_assisted += 0.2 * abs(delta_psi_threat) / pi
                if missile_in_mid_term:
                    reward_assisted -= 0.3 * abs(alpha-pi/3)/(pi/3)
            

        # 迎角惩罚
        reward_assisted -= 0.02 * ((ego.alpha_air*180/pi> 15)*(ego.alpha_air*180/pi-15)+\
                           (ego.alpha_air*180/pi< -5)*(-5 - ego.alpha_air*180/pi))
        reward_assisted -= 0.02 * (abs(ego.theta)/pi*2)  # 俯仰角惩罚
