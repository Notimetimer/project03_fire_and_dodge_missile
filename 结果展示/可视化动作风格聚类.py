...
def run_offline_tactical_analysis(json_path="Elite_Fire_Stats.json", n_clusters=4, weights=None):
    ...
    if X_weighted.shape[1] >= 3 and len(X_raw) >= 3:
        ...
        ax_3d.set_title(f'3D PCA Space Projection (Retains {var_3d:.1f}% Variance)', fontsize=13, weight='bold')
    ...
    if X_weighted.shape[1] >= 2 and len(X_raw) >= 2:
        ...
        ax_2d.set_title(f'2D PCA Space Projection (Retains {var_2d:.1f}% Variance)', fontsize=13, weight='bold')
