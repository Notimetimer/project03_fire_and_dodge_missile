...
TEAM_SIZE = 10        # 每组按 txt 文件顺序选取的成员数量
using_explore_maneuver = 1  # 是否在实验间测试的时候允许动作有随机性

# --- 2. 核心辅助函数 ---

def load_team_from_txt(log_dir, max_members=None):
    """
    从 log_dir/top_batch_names.txt 读取预设队伍名称，
    构造完整 .pt 路径，只保留文件实际存在的。
    max_members: 若指定，按文件顺序只取前 max_members 个有效路径。
    """
    ...
    if max_members is not None:
        agents_paths = agents_paths[:max_members]
...
def run_battle(env, blue_wrapper, red_wrapper, device):
    ...
    for count in range(int(20*60/env.dt_maneuver)):
        ...
        if count % action_cycle_multiplier == 0:
            ...
            with torch.no_grad():
                explore_dict = {'cat':1, 'bern':1} # {'cat': using_explore_maneuver, 'bern': 1}
                # cat 温度调低以凸显确定性, bern 保持1.0不受干扰
                temp_dict = {'cat': 1.0, 'bern': 1.0}  # cat: 0.5, bern: 0.97
        ...
        if getattr(env.RUAV, 'about_to_fire', 0):
            r_m_id = launch_missile_immediately(env, 'r', action_label=None, tabu=1) # action_label=r_label
        if getattr(env.BUAV, 'about_to_fire', 0):
            b_m_id = launch_missile_immediately(env, 'b', action_label=None, tabu=1) # action_label=r_label
...
if __name__ == "__main__":
    ...
    # 1. 从各任务目录的 top_batch_names.txt 加载预设队伍
    teams = []
    print("正在从 top_batch_names.txt 加载各任务预设队伍...")
    for name in mission_names:
        ...
        teams.append(load_team_from_txt(log_dir, max_members=TEAM_SIZE))
    ...
    for idx, (name, team) in enumerate(zip(mission_names, teams)):
        print(f"  [{idx}] {team_labels[idx]}  共 {len(team)} 个智能体")
    print(f"{'='*65}")
    input("\n确认以上信息无误，按 Enter 开始对抗...")

    # 并行配置
    num_processes = min(cpu_count(), 20)
    print(f"\n开始SOTA任务 [{team_labels[sota_idx]}] 对抗其他任务...")
    ...
    ax.set_ylabel('Rate (%)')
    ax.set_title(f'Combat Results (SOTA: {team_labels[sota_idx]})')
    ax.set_xticks(x)
