...
from Trains.ChoosingStrategyTrain_Big_dt_load_agents import *
import re
...
if __name__=="__main__":
    ...
    # 加载红方actor
    log_dir1 = os.path.join(pre_log_dir, "CombatFSP-run-20251117-192217")
    rein_list = glob.glob(os.path.join(log_dir1, "actor_rein*.pt"))
    latest_actor_path = latest_actor_by_index(rein_list)
    sd = th.load(latest_actor_path, map_location=device)  # sd 已为 state_dict
    red_actor.load_state_dict(sd)
    red_actor.eval() # 设置为评估模式

    # 加载蓝方actor MAPPO似乎用了prelustate，无法加载
    log_dir2 = os.path.join(pre_log_dir, "CombatFSP-run-20251114-110706")
