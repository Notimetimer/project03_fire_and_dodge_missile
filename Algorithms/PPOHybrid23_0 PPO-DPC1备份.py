...
class PolicyNetHybrid(torch.nn.Module):
    ...
    def forward(self, x, min_std=1e-6, max_std=1.0, action_masks=None):
        ...
        if 'cat' in self.action_dims and sum(self.action_dims['cat']) > 0:
            ...
            temperatures = 1.0  # use scalar temperature (or replace with tensor if per-head temperatures are needed)
            ...
            for i, logits in enumerate(cat_logits_list):
                ...
                scaled_logits = logits / (temperatures + 1e-8)
        ...
        if 'bern' in self.action_dims and self.action_dims['bern'] > 0:
            ...
            # use scalar temperature (no tensor ops on plain number)
            temperatures = 1.0 # torch.exp(self.log_temp_bern)
...
class PPOHybrid:
    ...
    def mixed_update(self, transition_dict, il_transition_dict, init_il_transition_dict=None, eta=0.1,  # [新增] 专家数据与约束系数
                     ...
                     # IL 参数
                     beta=1.0, il_batch_size=None, alpha=1.0, c_v=1.0, label_smoothing=0.3, max_weight=100.0, il_updates=1,
