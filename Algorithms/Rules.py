...
def decision_rule(ego_pos_, ego_psi, enm_pos_, distance, ally_missiles, enm_missiles, o00, R_cage, wander=0):
    ...
    action_n[0] = 7e3-ego_height # 固定高度为7000m
    ...
    # 2 crank
        ...
        if 0 <= delta_psi:
            ...
            action_n[1] = np.clip(temp, -0.4, 0.4)
        if delta_psi < 0:
            temp = 0.4 * (delta_psi + pi / 4) / (pi / 4) * 2
            action_n[1] = np.clip(temp, -0.4, 0.4)
        action_n[2] = 1.1*340  # 速度往1.1Ma去
    ...
    # 追踪任务的目标在散步
    if wander:
        action_n[0] = 3000 * np.random.uniform(-1,1)
        action_n[1] = np.random.normal(0, 25*pi/180)
        action_n[2] = 300
