...
class HybridActorWrapper(nn.Module):
    ...
    def evaluate_actions(self, states, actions_raw, h=None, max_std=None, mask_on=0, include_bern=1):
        """
        训练接口。计算 log_probs 和 entropy。
        Args:
            states: tensor (B, D)
            actions_raw: dict of tensors
            include_bern: 为0时 bern 头不计入 log_probs（等价于其概率恒为1），
                          熵仍照常记录；PPO 中用于跳过开火头更新
        Returns:
...
class PPOHybrid:
    ...
    def update(self, transition_dict, adv_normed=False,
                ...
                AFiltered=0, conf_thres=0.7, bern_included=0, ppo_with_bern=1):
                # [新增] ppo_with_bern=0 时 log_prob 不计入 bern 头（等价其概率恒为1），PPO 不再更新开火头 fc_bern
        ...
        # [新增] ppo_with_bern=0 时让 evaluate_actions 跳过 bern 头的 log_prob
        bern_on = 1 if ppo_with_bern else 0

        # 3. 计算旧策略的 log_probs (使用 Wrapper)
        with torch.no_grad():
            # Actor 使用 actor_inputs (可能是 obs)
            # [修改] 接收 5 个返回值
            old_log_probs, _, _, _ ,_ = self.actor.evaluate_actions(actor_inputs, actions_on_device, h=None, max_std=self.max_std, mask_on=mask_on, include_bern=bern_on)
        ...
        for _ in range(self.epochs):
            ...
            for start in range(0, num_samples, mini_batch_size):
                ...
                log_probs, entropy, entropy_details, actor_outputs, _ = self.actor.evaluate_actions(mb_actor_inputs, mb_actions, h=None, max_std=self.max_std, mask_on=mask_on, include_bern=bern_on)
