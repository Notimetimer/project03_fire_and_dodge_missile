...
class PolicyNetHybrid(torch.nn.Module):
    ...
    def __init__(self, state_dim, hidden_dims, action_dims_dict, init_std=0.5):
        ...
        if 'bern' in self.action_dims and self.action_dims['bern'] > 0:
            bern_layers = []
            bern_prev_size = state_dim
            for layer_size in hidden_dims:
                bern_layers.append(nn.Linear(bern_prev_size, layer_size))
                bern_layers.append(nn.ReLU())
                bern_prev_size = layer_size
            self.bern_net = nn.Sequential(*bern_layers)
        ...
        if 'bern' in self.action_dims and self.action_dims['bern'] > 0:
            ...
            # 原·单层输出
            self.fc_bern = nn.Linear(bern_prev_size, bern_dim)
            # # 现·多层输出
            # layers = []
            # for _ in range(1):
            #     layers.append(nn.Linear(bern_prev_size, 64))
            #     layers.append(nn.ReLU())
            #     bern_prev_size = 64
            # layers.append(nn.Linear(bern_prev_size, bern_dim))
    ...
    def set_bern_requires_grad(self, requires_grad: bool):
        """独立控制伯努利分支（独立主干与输出层）是否参与梯度更新"""
        if hasattr(self, 'bern_net'):
            for param in self.bern_net.parameters():
                param.requires_grad = requires_grad
        if hasattr(self, 'fc_bern'):
            for param in self.fc_bern.parameters():
                param.requires_grad = requires_grad
    ...
    def forward(self, x, min_std=1e-6, max_std=1.0, action_masks=None, temp=1.0):
        ...
        # --- Bernoulli (核心修改区域) ---
        if 'bern' in self.action_dims and self.action_dims['bern'] > 0:
            bern_features = self.bern_net(x)
            bern_logits = self.fc_bern(bern_features)
...
class HybridActorWrapper(nn.Module):
    ...
    def set_bern_requires_grad(self, requires_grad: bool):
        """透传：独立控制伯努利分支是否参与梯度更新"""
        if hasattr(self.net, 'set_bern_requires_grad'):
            self.net.set_bern_requires_grad(requires_grad)
...
class PPOHybrid:
    ...
    def set_bern_requires_grad(self, requires_grad: bool):
        """透传：独立控制 Actor 伯努利分支是否参与梯度更新"""
        if hasattr(self.actor, 'set_bern_requires_grad'):
            self.actor.set_bern_requires_grad(requires_grad)
