...
class PolicyNetHybrid(torch.nn.Module):
    ...
    def __init__(self, state_dim, hidden_dims, action_dims_dict, init_std=0.5, head_hidden_layer_num=1, Autoregressive=0,
                 gru_hidden_size=128, gru_num_layers=1):
    ...
    # [修改] 增加 action_masks / h / return_h 参数, [新增] 支持时序输入
    def forward(self, x, h=None, min_std=1e-6, max_std=1.0, action_masks=None, temperature=1.0, mask_on=0, return_h=False):
...
class HybridActorWrapper(nn.Module):
    ...
    def _prepare_h(self, h, batch_size):
        """将 h 统一转换为 (num_layers, batch, hidden) 的 Tensor"""
        if h is None:
            return None
        if not isinstance(h, torch.Tensor):
            ...
            h = h.to(self.device)
        # 若 h 是 (num_layers, hidden)，扩展 batch 维度
        if h.dim() == 2:
            h = h.unsqueeze(1)
        # 若 batch 不匹配，但 shape 为 (L, 1, H)，广播到 (L, batch, H)
        if h.dim() == 3 and h.size(1) == 1 and batch_size > 1:
            h = h.expand(-1, batch_size, -1)
        return h

    # [修改] 增加 check_obs 参数，默认为 None， [新增] 增加 temperature / h 参数
    def get_action(self, state, h=None, explore=True, max_std=None, check_obs=None, bern_threshold=0.5, temperature=1.0, mask_on=0): # 1
        ...
        Args:
            ...
            h: hidden state (num_layers, hidden) 或 (num_layers, batch, hidden)
        ...
        Returns:
            ...
            next_h: hidden state (num_layers, batch, hidden)
        ...
        h = self._prepare_h(h, state.size(0))
        ...
        actor_outputs, h_out = self.net(state, h=h, max_std=max_std, temperature=temperature, mask_on=mask_on, return_h=True)
    ...
    def evaluate_actions(self, states, actions_raw, h=None, max_std=None, mask_on=0):
        ...
        Args:
            states: tensor (B, D) 或 (B, T, D)
            actions_raw: dict of tensors，每个值形状 (B, T, ...) 或 (B*T, ...)
            h: hidden state (num_layers, B, hidden)
        Returns:
            log_probs: tensor (B*T, 1)
            entropy: tensor (B*T, 1)
            entropy_details: dict
            actor_outputs: dict (raw outputs from net)
            h_out: hidden state
        """
        ...
        B, T, D = states.shape
        h = self._prepare_h(h, B)

        # 铺平时序维度的动作
        actions_flat = {}
        for k, v in actions_raw.items():
            if not isinstance(v, torch.Tensor):
                v = torch.tensor(np.array(v), dtype=torch.long if k == 'cat' else torch.float).to(self.device)
            if v.dim() >= 3 and v.size(0) == B and v.size(1) == T:
                # (B, T, *) -> (B*T, *)
                actions_flat[k] = v.reshape(B * T, *v.shape[2:])
            else:
                actions_flat[k] = v

        actor_outputs, h_out = self.net(states, h=h, max_std=max_std, mask_on=mask_on, return_h=True)

        total_steps = B * T
        log_probs = torch.zeros(total_steps, 1).to(self.device)
        entropy = torch.zeros(total_steps, 1).to(self.device)
        ...
        if 'cat' in self.action_dims and sum(self.action_dims['cat']) > 0:
            cat_probs_list = actor_outputs['cat']
            cat_action = actions_flat['cat'].long()
        ...
        if 'bern' in self.action_dims and self.action_dims['bern'] > 0:
            ...
            dist = Bernoulli(logits=bern_logits)
            bern_action = actions_flat['bern']
            log_probs += dist.log_prob(bern_action).sum(-1, keepdim=True)
            e_bern = dist.entropy().sum(-1, keepdim=True)
            entropy += e_bern
            entropy_details['bern'] = e_bern

        return log_probs, entropy, entropy_details, actor_outputs, h_out
    
    def compute_il_loss(self, states, expert_actions, h=None, label_smoothing=0.3, action_heads_mask=None, no_bern=None, no_cat=None, mask_on=0, good_samples=1, pre_training=1):
        """
        计算模仿学习 Loss (MARWIL / BC)。
        
        Args:
            states: (Batch, State_Dim)
            h: hidden state, 默认 None 时内部自动初始化为全 0
        ...
        if not isinstance(states, torch.Tensor):
            states = torch.tensor(states, dtype=torch.float).to(self.device)
        for k in expert_actions:
            if not isinstance(expert_actions[k], torch.Tensor):
                dtype = torch.long if k == 'cat' else torch.float
                expert_actions[k] = torch.tensor(expert_actions[k], dtype=dtype).to(self.device)

        h = self._prepare_h(h, states.size(0))
        actor_outputs, _ = self.net(states, h=h, mask_on=mask_on, return_h=True)
    ...
    @torch.no_grad()
    def compute_marwil_monitor(self, states, expert_actions, h=None, mask_on=0, advantages=None):
        """
...
class PPOHybrid:
    ...
    def update(self, transition_dict, adv_normed=False, 
        ...
        # 兼容 2D (N, D) 与 3D (B, T, D)，统一按序列处理
        if states.dim() == 2:
            states = states.unsqueeze(1)
        B, T, _ = states.shape

        # 初始隐藏状态：默认全 0
        def get_h(key, net, default_L, default_H):
            if key in transition_dict and transition_dict[key] is not None:
                h = to_tensor(transition_dict[key], torch.float)
                if h.dim() == 3 and h.size(0) == B:
                    # (B, L, H) -> (L, B, H)
        ...
        rewards = to_tensor(transition_dict['rewards'], torch.float).view(B, T, 1)

        #  处理 active_masks (可选输入)
        # 如果 transition_dict 中没有 active_masks，则默认所有样本均有效
        if 'active_masks' in transition_dict:
            active_masks = to_tensor(transition_dict['active_masks'], torch.float).view(B, T, 1)
        else:
        ...
        if 'obs' in transition_dict:
            ...
            if actor_inputs.dim() == 2:
                actor_inputs = actor_inputs.unsqueeze(1)
        ...
        def _prepare_action_tensor(t, key):
            if key == 'cat':
                dtype = torch.long
            else:
                dtype = torch.float
            if not isinstance(t, torch.Tensor):
                t = torch.tensor(np.array(t), dtype=dtype).to(self.device)
            else:
                t = t.to(self.device)
            # 兼容 2D (N, dim) -> (N, 1, dim) 与 3D (B, T, dim)
            if t.dim() == 2:
                t = t.unsqueeze(1)
            return t
        ...
        if isinstance(actions_from_buffer, dict):
            for key, val in actions_from_buffer.items():
                actions_on_device[key] = _prepare_action_tensor(val, key)
        else:
            # 兼容旧代码 (list of dicts)
            all_keys = actions_from_buffer[0].keys()
            for key in all_keys:
                vals = [d[key] for d in actions_from_buffer]
                actions_on_device[key] = _prepare_action_tensor(vals, key)


        # 2. 获取 Advantage (优先使用 Buffer 算好的)
        if 'advantages' in transition_dict and 'td_targets' in transition_dict:
            advantage = to_tensor(transition_dict['advantages'], torch.float).view(B, T, 1)
            td_target = to_tensor(transition_dict['td_targets'], torch.float).view(B, T, 1)
        else:
            # 现场计算 GAE：展开成 1D 计算后再 reshape 回 (B, T, 1)
            if 'truncs' in transition_dict:
                truncs = to_tensor(transition_dict['truncs'], torch.float).view(B, T, 1)
            else:
                truncs = None

            with torch.no_grad():
                if 'next_values' in transition_dict:
                    next_vals = to_tensor(transition_dict['next_values'], torch.float).view(B, T, 1)
                else:
                    next_states = to_tensor(transition_dict['next_states'], torch.float).view(B, T, -1)
                    next_vals = self.critic(next_states, h=h_critic)

                td_target = rewards + self.gamma * next_vals.view(B, T, 1) * (1 - dones)
                critic_vals = self.critic(critic_inputs, h=h_critic).view(B, T, 1)
                td_delta = td_target - critic_vals
                td_delta_flat = td_delta.reshape(-1, 1).cpu()
                dones_flat = dones.reshape(-1, 1).cpu()
                truncs_flat = truncs.reshape(-1, 1).cpu() if truncs is not None else None
                advantage_flat = compute_advantage(self.gamma, self.lmbda, td_delta_flat, dones_flat, truncs_flat).to(self.device)
                advantage = advantage_flat.view(B, T, 1)
                td_target = td_target.view(B, T, 1)

        # 3. 计算旧策略的 log_probs (使用 Wrapper) —— 现在在每个 mini-batch 中按序列重新计算
        v_pred_old = self.critic(critic_inputs, h=h_critic).reshape(-1, 1)
        ...
        for _ in range(self.epochs):
            ...
            for start in range(0, num_samples, mini_batch_size):
                ...
                # 切片 mini-batch 数据
                mb_actor_inputs = actor_inputs[batch_idx]          # (b, T, obs_dim)
                mb_critic_inputs = critic_inputs[batch_idx]        # (b, T, state_dim)
                mb_advantage = advantage[batch_idx].reshape(-1, 1)   # (b*T, 1)
                mb_td_target = td_target[batch_idx].reshape(-1, 1)   # (b*T, 1)
                mb_active_masks = active_masks[batch_idx].reshape(-1, 1)  # (b*T, 1)

                # 切片 Actions (Dict 结构)
                mb_actions = {}
                for k, v in actions_on_device.items():
                    mb_actions[k] = v[batch_idx]                    # (b, T, ...)

                # 切片隐状态 (L, B, H) -> (L, b, H)
                mb_h_actor = h_actor[:, batch_idx, :]
                mb_h_critic = h_critic[:, batch_idx, :]

                # 计算旧策略 log_probs (no_grad)
                with torch.no_grad():
                    mb_old_log_probs, _, _, _, _ = self.actor.evaluate_actions(
                        mb_actor_inputs, mb_actions, h=mb_h_actor,
                        max_std=self.max_std, mask_on=mask_on
                    )
                ...
                v_pred = self.critic(mb_critic_inputs, h=mb_h_critic)
                # if clip_vf:
        ...
        mask_bool = active_masks.squeeze(-1).bool().cpu().numpy().flatten()
        y_true = td_target.flatten().detach().cpu().numpy()[mask_bool]
        y_pred = v_pred_old.flatten().detach().cpu().numpy()[mask_bool] # 比较更新前的 Value 网络预测能力
    ...
    def fire_prob_protection(self, transition_dict, protect_epochs=4, protect_mini_batch=256, mask_on=0):
        ...
        # fire_prob_protection 不需要序列隐状态，只需逐步有监督；兼容 (N, D) 与 (B, T, D)
        if actor_inputs.dim() == 3:
            actor_inputs = actor_inputs.reshape(-1, actor_inputs.size(-1))
    ...
    def reshape_for_rnn(self, transition_dict, seq_len):
        ...
        Args:
            transition_dict: dict，键包含
                ...
                可选: 'actor_h' (N, L_a, H_a), 'critic_h' (N, L_c, H_c)
            seq_len: int，每条序列的长度

        Returns:
            dict，各字段形状为 (num_seqs, seq_len, ...) 的 numpy 数组；
            'actions' 仍为 dict，每个值形状 (num_seqs, seq_len, ...)。
            额外包含 'init_h_actor' (num_seqs, L_a, H_a) 和 'init_h_critic' (num_seqs, L_c, H_c)。
        ...
        has_actor_h      = 'actor_h' in transition_dict and transition_dict['actor_h'] is not None
        has_critic_h     = 'critic_h' in transition_dict and transition_dict['critic_h'] is not None
        ...
        actor_h      = _to_np(transition_dict['actor_h'])      if has_actor_h      else None
        critic_h     = _to_np(transition_dict['critic_h'])     if has_critic_h     else None
        ...
        if has_actor_h:
            ...
            out['init_h_actor'] = np.zeros((num_seqs, L_a, H_a), dtype=np.float32)
        if has_critic_h:
            out['init_h_critic'] = np.zeros((num_seqs, L_c, H_c), dtype=np.float32)
        else:
            out['init_h_critic'] = np.zeros((num_seqs, L_c, H_c), dtype=np.float32)
        ...
        for i, (s, e) in enumerate(valid_seqs):
            ...
            # 序列首帧之前的隐藏状态
            if has_actor_h:
                out['init_h_actor'][i] = actor_h[s]
            if has_critic_h:
                out['init_h_critic'][i] = critic_h[s]
    ...
    def MARWIL_update(self, il_transition_dict, beta=1.0, batch_size=64, alpha=1.0, c_v=1.0, shuffled=1, label_smoothing=0.3, max_weight=100.0,
        ...
        # 模仿学习样本若未提供 hidden state，则默认全 0
        def get_h(key, default_L, default_H, N):
            if key in il_transition_dict and il_transition_dict[key] is not None:
                h = torch.tensor(np.array(il_transition_dict[key]), dtype=torch.float).to(self.device)
                if h.dim() == 2:
                    h = h.unsqueeze(1).expand(-1, N, -1)
                elif h.dim() == 3 and h.size(0) == N:
                    # 可能是 (N, L, H)，转换为 (L, N, H)
                    h = h.permute(1, 0, 2)
                return h
        ...
        for start in range(0, total_size, batch_size):
            ...
            # 隐状态切片
            h_actor_batch = h_actor_all[:, batch_indices, :]
            h_critic_batch = h_critic_all[:, batch_indices, :]

            # A. Advantage & Weights
            with torch.no_grad():
                values = self.critic(s_batch, h=h_critic_batch)
            ...
            # C. Critic Loss
            v_pred = self.critic(s_batch, h=h_critic_batch)
