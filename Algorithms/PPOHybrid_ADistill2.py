...
class HybridActorWrapper(nn.Module):
    ...
    def evaluate_actions(self, states, actions_raw, h=None, max_std=None, mask_on=0):
        ...
        Returns:
            ...
            entropy: tensor (B, 1)
            entropy_details: dict, 分项熵 {'cont','cat','bern'}
            actor_outputs: dict (raw outputs from net) [新增]
            logprob_details: dict, 分项 log_prob {'cont','cat','bern'} [新增，供 ADistill 补偿项计算 cat/bern 各自新旧策略比使用]
        """
        ...
        #  [新增] 用于记录分项 log_prob 的字典 (供 ADistill 补偿项单独计算 cat/bern 的新旧策略比使用)
        logprob_details = {'cont': None, 'cat': None, 'bern': None}
        ...
        if 'cont' in self.action_dims and self.action_dims['cont'] > 0:
            ...
            dist = SquashedNormal(mu, std)
            u = actions_raw['cont']
            lp_cont = dist.log_prob(0, u).sum(-1, keepdim=True)
            log_probs += lp_cont
            logprob_details['cont'] = lp_cont
        ...
        if 'cat' in self.action_dims and sum(self.action_dims['cat']) > 0:
            ...
            #  临时列表用于计算 cat 总熵 / 总 log_prob
            e_cat_sum = torch.zeros_like(entropy)
            lp_cat_sum = torch.zeros_like(log_probs)
            
            for i, probs in enumerate(cat_probs_list):
                act_i = cat_action[:, i].unsqueeze(-1)
                lp_head = torch.log(probs.gather(1, act_i) + 1e-8)
                log_probs += lp_head
                lp_cat_sum += lp_head
                dist = Categorical(probs=probs)
            ...
            #  记录 cat entropy / log_prob
            entropy_details['cat'] = e_cat_sum # [修改] 保持 Tensor 用于 Loss 计算
            logprob_details['cat'] = lp_cat_sum
        ...
        if 'bern' in self.action_dims and self.action_dims['bern'] > 0:
            ...
            lp_bern = dist.log_prob(bern_action).sum(-1, keepdim=True)
            log_probs += lp_bern
            logprob_details['bern'] = lp_bern
        ...
        # [修改] 返回 actor_outputs 以便外部访问 logits；[新增] 返回 logprob_details 供 ADistill 补偿项使用
        return log_probs, entropy, entropy_details, actor_outputs, logprob_details
...
class PPOHybrid:
    ...
    def ADistill(self, mb_states, mb_actions, mb_active_masks, mb_advantage,
                 alpha_distill_cat=0, alpha_distill_bern=0,
                 teacher_actor=None, AFiltered=0, conf_thres=0.7, bern_included=0):
        """
        (Policy Distillation, 分项版):
        根据 teacher_actor 在 mb_states 上的预测，分别对 cat 和 bern 两组动作独立判断
        实际执行的动作是否与 teacher 一致，一致则分别记为 alpha_distill_cat / alpha_distill_bern 分
        （不一致记 0 分），返回 {"cat": cat_score, "bern": bern_score}（均为 (B, 1) 的未归一化分值张量）。
        该分值不再乘以 advantage.std()，因为本函数现在在 update 的 epoch/mini-batch 循环内、
        advantage 已完成归一化之后调用；调用方会用这两组分值分别乘以 cat / bern 各自的新旧
        策略比，作为 Actor Loss 中独立于 Advantage 项的补偿项。
        conf_thres 是一道用于防止过度诱导的锁：当 student 执行 teacher 相同 cat 动作组合的联合概率超过
        conf_thres 时，就不再对该样本的 cat 加分（如果要全程引导，把 conf_thres 设置为 >=1）。
        bern_included 控制 bern 分组是否参与打分（=0 时 bern_score 全 0，即不学开火）。
        AFiltered=1 时，只对那些优势度高于均值（mb_advantage > mean(mb_advantage)）的样本打分。
        """
        zero_score = torch.zeros_like(mb_advantage)
        if teacher_actor is None:
            return {"cat": zero_score, "bern": zero_score}

        active = mb_active_masks.bool().squeeze(-1)

        # 实际 cat / bern 动作
        cat_actions = mb_actions.get('cat')
        bern_actions = mb_actions.get('bern')
        if cat_actions is not None and cat_actions.dim() == 1:
            cat_actions = cat_actions.unsqueeze(-1)
        ...
        with torch.no_grad():
            if getattr(teacher_actor, 'is_rule_teacher', False):
                teacher_outputs = teacher_actor.predict_distributions(mb_states)
            else:
                teacher_outputs = teacher_actor.net(mb_states)

            # 同时调用 student actor，获取其在当前 obs 下的动作分布（用于 conf_thres 判断）
            student_outputs = self.actor.net(mb_states)

            teacher_cat = teacher_outputs.get('cat')
            student_cat = student_outputs.get('cat')

            # AFiltered 共用的优势度筛选 mask
            adv_pos = (mb_advantage > mb_advantage.mean()).squeeze(-1) if AFiltered else None

            # --- Cat 分组打分：逐个 head 判断实际动作是否与 teacher 一致 ---
            cat_score = zero_score
            if cat_actions is not None and teacher_cat is not None and len(teacher_cat) > 0 \
               and student_cat is not None and len(student_cat) > 0:
                cat_match = active.clone()
                joint_p = torch.ones(mb_advantage.size(0), device=self.device, dtype=torch.float)
                for h, teacher_probs in enumerate(teacher_cat):
                    ...
                    cat_match = cat_match & (student_act == teacher_act)
                    joint_p = joint_p * p_student
                cat_match = cat_match & (joint_p <= conf_thres)

                if AFiltered:
                    cat_match = cat_match & adv_pos

                cat_score = cat_match.float().view_as(mb_advantage) * alpha_distill_cat

            # --- Bern 分组打分：只匹配 bern 头动作，与 cat 分组独立 ---
            bern_score = zero_score
            if bern_included and bern_actions is not None and teacher_outputs.get('bern') is not None:
                ...
                bern_match = active & (bern_actions == teacher_bern_act).all(dim=-1)

                if AFiltered:
                    bern_match = bern_match & adv_pos

                bern_score = bern_match.float().view_as(mb_advantage) * alpha_distill_bern

        return {"cat": cat_score, "bern": bern_score}
    ...
    def update(self, transition_dict, adv_normed=False, 
                ...
                alpha_distill_cat=None, alpha_distill_bern=None, use_cat_distill=True, use_bern_distill=True): 
                # [新增] target_p1 默认“一超”概率，剩下来的留给“多强”)
                # [修改] 增加 target_p1_b 参数，对应开火控制的“笃定程度”

        # 解析 cat/bern 各自的蒸馏强度。若未显式设置，则回退到 alpha_distill。
        _ad_cat = alpha_distill if alpha_distill_cat is None else alpha_distill_cat
        _ad_bern = alpha_distill if alpha_distill_bern is None else alpha_distill_bern
        ...
        with torch.no_grad():
            ...
            # [修改] 接收 5 个返回值；[新增] old_logprob_details 供 ADistill 补偿项计算 cat/bern 各自新旧策略比使用
            old_log_probs, _, _, _, old_logprob_details = self.actor.evaluate_actions(actor_inputs, actions_on_device, h=None, max_std=self.max_std, mask_on=mask_on)
        ...
        for _ in range(self.epochs):
            ...
            for start in range(0, num_samples, mini_batch_size):
                ...
                #  切片旧策略的分项 log_prob (cat/bern)，供 ADistill 补偿项计算各自新旧策略比
                mb_old_log_probs_cat = old_logprob_details['cat'][batch_idx] if old_logprob_details.get('cat') is not None else None
                mb_old_log_probs_bern = old_logprob_details['bern'][batch_idx] if old_logprob_details.get('bern') is not None else None
                ...
                #  接收 entropy_details、actor_outputs 和 logprob_details
                log_probs, entropy, entropy_details, actor_outputs, logprob_details = self.actor.evaluate_actions(mb_actor_inputs, mb_actions, h=None, max_std=self.max_std, mask_on=mask_on)
                ...
                # =====================================================================
                # [新增] 策略蒸馏补偿项 (ADistill)：与 teacher 一致的 cat/bern 动作分别加分
                # Actor_loss 现分为三部分：归一化 Advantage 的 surrogate 项(以上) + 不归一化的
                # cat 相似度补偿项 + bern 相似度补偿项。cat 补偿项仅用 cat 部分的新旧策略比，
                # bern 补偿项仅用 bern 部分的新旧策略比；相同动作加分，不同动作不加分的逻辑不变。
                # 因为在这里 Advantage 已经完成归一化，不再需要 advantage.std() 参与缩放。
                # =====================================================================
                if (_ad_cat > 0 or _ad_bern > 0) and teacher_actor is not None and (use_cat_distill or use_bern_distill):
                    distill_scores = self.ADistill(mb_actor_inputs, mb_actions, mb_active_masks, mb_advantage,
                                                    alpha_distill_cat=_ad_cat, alpha_distill_bern=_ad_bern,
                                                    teacher_actor=teacher_actor, AFiltered=AFiltered,
                                                    conf_thres=conf_thres, bern_included=bern_included)

                    # --- Cat 相似度补偿项：仅对 cat 部分计算新旧策略比 ---
                    cat_score = distill_scores.get('cat') if use_cat_distill else None
                    if cat_score is not None and mb_old_log_probs_cat is not None and logprob_details.get('cat') is not None:
                        ratio_cat = torch.exp(logprob_details['cat'] - mb_old_log_probs_cat)
                        if v_trace is not None:
                            ratio_cat = torch.clamp(ratio_cat, max=v_trace)
                        cat_distill_loss = -(ratio_cat * cat_score * mb_active_masks).sum() / (active_sum + mask_eps)
                        actor_loss = actor_loss + cat_distill_loss

                    # --- Bern 相似度补偿项：仅对 bern 部分计算新旧策略比 ---
                    bern_score = distill_scores.get('bern') if use_bern_distill else None
                    if bern_score is not None and mb_old_log_probs_bern is not None and logprob_details.get('bern') is not None:
                        ratio_bern = torch.exp(logprob_details['bern'] - mb_old_log_probs_bern)
                        if v_trace is not None:
                            ratio_bern = torch.clamp(ratio_bern, max=v_trace)
                        bern_distill_loss = -(ratio_bern * bern_score * mb_active_masks).sum() / (active_sum + mask_eps)
                        actor_loss = actor_loss + bern_distill_loss
