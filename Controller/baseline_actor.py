...
class ACTLayer(nn.Module):
    def __init__(self, input_dims, action_dims, use_mlp_actlayer=False):
        """
        input_dims: List[int], 每个动作输出层的输入维度
        action_dims: List[int], 每个动作输出层的输出维度
        use_mlp_actlayer: bool, 是否使用 MLP 层
        """
        super(ACTLayer, self).__init__()
        self._mlp_actlayer = use_mlp_actlayer
        if self._mlp_actlayer:
            self.mlp = MLPLayer(input_dims[-1], '128 128')  # 使用最后一个输入维度作为 MLP 的输入

        # 动态创建每个动作输出层
        action_outs = []
        for input_dim, action_dim in zip(input_dims, action_dims):
    ...
    def forward(self, x):
        ...
        dim=-1 表示沿着最后一个维度（列方向）拼接
        拼接后的张量形状为 [batch_size, sum(action_dims)]，例如：
        [[20, 18, 20, 15],  # 样本1的动作值
         [25, 22, 25, 20]]  # 样本2的动作值
        '''
        return actions
...
class BaselineActor(nn.Module):
    def __init__(self, input_dim=12, use_mlp_actlayer=False) -> None:
        ...
        if not use_mlp_actlayer:
            self.act = ACTLayer([128], [41, 41, 41, 30], use_mlp_actlayer)
        else:
            self.act = ACTLayer([128, 128, 128, 128], [41, 41, 41, 30], use_mlp_actlayer)
