...
class ChooseStrategyEnv(Battle):
    ...
    def combat_terminate_and_reward(self, side, action_label, action_shoot):
        ...
        reward_fly += ((alt <= self.min_alt_safe) * np.clip(ego.vu / 100, -1, 1) + \
                (alt >= self.max_alt_safe) * np.clip(-ego.vu / 100, -1, 1)) * 5
        
        # 靠近边界惩罚
        if d_hor < 20e3:
            reward_main += (d_hor/20e3-1) * 2
            reward_fly += (d_hor/20e3-1) * 2
        ...
        if len(alive_ally_missiles) == 0 and ego.ammo > 0 and not warning:
            ...
            reward_fly += 0.5 - alpha / pi
            
        
        # # 有warning时alpha越大越好
        if warning:
            reward_assisted += 0.8 * abs(delta_psi_threat) / pi
            reward_fly += 0.8 * abs(delta_psi_threat) / pi
        ...
        reward_fly -= 0.1 * ((ego.alpha_air*180/pi> 15)*(ego.alpha_air*180/pi-15)+\
                           (ego.alpha_air*180/pi< -5)*(-5 - ego.alpha_air*180/pi))-\
                               0.1 * (abs(ego.theta)/pi*2)  # 俯仰角惩罚
        ...
        return done, reward_main+reward_assisted, reward_assisted, reward_fly, reward_fire
