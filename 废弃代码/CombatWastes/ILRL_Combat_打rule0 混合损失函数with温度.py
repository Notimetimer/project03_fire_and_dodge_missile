...
if __name__ == "__main__":
    ...
    # =========================================================================
    # [修改 1] 预处理 IL 数据结构：1. 替换键名； 2. 封装为 np.array
    # =========================================================================
    if 'actions' in il_transition_dict and isinstance(il_transition_dict['actions'], list):
        print("Preprocessing IL actions: 1. Mapping keys ('fly'/'fire' -> 'cat'/'bern')...")
        print("2. Wrapping scalars in np.array()...")
        
        raw_actions_list = il_transition_dict['actions']
        new_actions_list = []
        
        for act_dict in raw_actions_list:
            new_act_dict = {}
            
            # --- 步骤 1: 键名替换 ---
            # 映射 'fly' -> 'cat' (离散机动)
            if 'fly' in act_dict:
                act_value = act_dict['fly']
                # --- 步骤 2: 数据封装 ---
                # 离散动作转 int64
                new_act_dict['cat'] = np.array([act_value], dtype=np.int64)
            
            # 映射 'fire' -> 'bern' (伯努利开火)
            if 'fire' in act_dict:
                act_value = act_dict['fire']
                # --- 步骤 2: 数据封装 ---
                # 伯努利动作转 float32
                new_act_dict['bern'] = np.array([act_value], dtype=np.float32)
                
            new_actions_list.append(new_act_dict)
            
        il_transition_dict['actions'] = new_actions_list
        print(f"IL actions preprocessed. Final structure: List[Dict] with keys 'cat'/'bern'. Count: {len(il_transition_dict['actions'])}")

    summarize(il_transition_dict)

    # 日志记录 (使用您自定义的 TensorBoardLogger)
    logs_dir = os.path.join(project_root, "logs/combat")
    mission_name = 'ILRL_combat_打rule0混合损失函数with温度'
